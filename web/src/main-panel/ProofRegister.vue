...
<template>
  <section class="proof-register">
    ...
    <TransitionGroup name="proof-insert" tag="div" class="proof-list">
      <details v-for="task in tasks" :key="task.id" class="proof">
        <summary>
        ...
        <div class="proof-body">
          ...
          <article v-else-if="task.agent_type === 'script'" class="copy">
            <template v-for="(b, i) in task.artifacts?.blocks || []" :key="i">
              <h3 v-if="b.kind === 'title'">{{ b.text }}</h3><h4 v-else-if="b.kind === 'heading'">{{ b.text }}</h4><blockquote v-else-if="b.kind === 'quote'">{{ b.text }}</blockquote><p v-else>{{ b.text }}</p>
            </template>
          </article>
...
.proof[open] .proof-mark::after { transform: rotate(90deg); }
.opened { display: none; }.proof[open] .closed { display: none; }.proof[open] .opened { display: inline; }
.proof-body { position: relative; padding: 22px 24px; border-top: 1px solid var(--ch-border); }
...
.copy h3 { margin: 0 0 18px; font: 700 24px/1.4 var(--ch-serif); }.copy h4 { margin: 18px 0 6px; font: 600 15px/1.5 var(--ch-serif); }.copy p { margin: 6px 0; color: var(--ch-body); font: 500 13px/1.85 var(--ch-serif); }.copy blockquote { margin: 16px 0; padding: 10px 14px; border-top: 1px solid var(--ch-border-2); border-bottom: 1px solid var(--ch-border-2); font: 600 14px/1.7 var(--ch-serif); text-align: center; }
