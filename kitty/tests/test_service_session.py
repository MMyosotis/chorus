...
/list（按 updated_at 倒序）/set_title_if_unset（已设则 no-op）/get_lock（同会话同锁对象、
...
from kitty.tests._helpers import fresh_conn
...
def test_set_title_if_unset_idempotent():
    svc = _svc()
    s = svc.create("hi")                               # title_generated=False
    assert svc.set_title_if_unset(s.id, "自动标题") is True
    got = svc.get(s.id)
    assert got.title == "自动标题"
    assert got.title_generated is True
    assert svc.set_title_if_unset(s.id, "另一个") is False   # 已设 → no-op
    assert svc.get(s.id).title == "自动标题"                 # 不变
    s2 = svc.create("x")
    assert svc.set_title_if_unset(s2.id, "   ") is False     # 归一为空 → 跳过
