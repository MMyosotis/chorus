...
def build_consolidate_prompt(memories: list[CreatorMemory]) -> str:
    ...
    return (
        ...
        "4. 整理到 20 条以内\n"
        "5. description 统一改写为「类型：要点」格式，不得超过 20 字\n"
        "6. 合并多条时取来源里最新的时间；未合并的保留原时间；created_at 原样抄回\n"
        "7. 返回整理后的 JSON 数组，每条包含 description, content, platform, visible_to, kind, created_at\n"
        "8. kind 只能是 \"performance\" 或 \"reference\"；created_at 格式 YYYY-MM-DD HH:MM；"
