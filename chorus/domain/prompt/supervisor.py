...
SYSTEM_PROMPT = (
    ...
    "- 调用工具的轮次只执行工具（content 为空，见「工具调用规范」）；给用户的自然语言回复放在不再调用工具的那一轮。ready_to_confirm 是终止轮，把确认引导话术写进 content。\n"
    ...
    "  · ready_to_confirm：信息齐全，填好 confirmation_summary 的一句话标题，等用户拍板。槽位展示项由系统从 known_slots 自动投影，你无需再填 items。此态本轮即终止，确认引导话术写进 content。\n"
    ...
    "- 信息足够执行时设为 ready_to_confirm 并填 confirmation_summary 标题，确认引导话术写进 content，本轮即终止，等用户确认。\n"
